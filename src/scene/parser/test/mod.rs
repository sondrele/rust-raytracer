...
fn can_parse_mesh() {
    ...
    assert_eq!(&poly0.x.material, &mesh.materials[0]);
    assert_eq!(&poly0.y.material, &mesh.materials[1]);
    assert_eq!(&poly0.z.material, &mesh.materials[2]);
    match poly0.x.normal {
        Some(ref n) => assert_eq!(n, &mesh.normals[0]), None => panic!("Should have normal")
    }
    match poly0.y.normal {
        Some(ref n) => assert_eq!(n, &mesh.normals[1]), None => panic!("Should have normal")
    }
    match poly0.z.normal {
        Some(ref n) => assert_eq!(n, &mesh.normals[2]), None => panic!("Should have normal")
    }

    let ref poly1 = mesh[1];
    assert_eq!(&poly1.x.material, &mesh.materials[3]);
    assert_eq!(&poly1.y.material, &mesh.materials[3]);
    assert_eq!(&poly1.z.material, &mesh.materials[4]);

    let ref poly2 = mesh[2];
    assert_eq!(&poly2.x.material, &mesh.materials[5]);
    assert_eq!(&poly2.y.material, &mesh.materials[4]);
    assert_eq!(&poly2.z.material, &mesh.materials[5]);
    match poly2.x.normal {
        Some(ref n) => assert_eq!(n, &mesh.normals[6]), None => panic!("Should have normal")
    }
    match poly2.y.normal {
        Some(ref n) => assert_eq!(n, &mesh.normals[7]), None => panic!("Should have normal")
    }
    match poly2.z.normal {
